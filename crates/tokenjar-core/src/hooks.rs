//! Transparent Shell Hooking Manager for TokenJar in Rust.
//!
...
pub const HOOK_MARKER_START: &str = "# >>> tokenjar-hook >>>";
pub const HOOK_MARKER_END: &str = "# <<< tokenjar-hook <<<";
...
pub fn generate_powershell_hook() -> String {
    let mut script = vec![
        ...
        "# TokenJar Transparent CLI Interceptor (PowerShell)".to_string(),
    ];

    for cmd in DEFAULT_WRAPPED_COMMANDS {
        script.push(format!(
            "function {cmd} {{\n    if ($env:RAW -eq '1' -or $env:TOKENJAR_BYPASS -eq '1') {{\n        & (Get-Command -CommandType Application {cmd} | Select-Object -First 1).Source @args\n    }} else {{\n        tokenjar run -- {cmd} @args\n    }}\n}}"
        ));
    }
...
pub fn generate_posix_hook() -> String {
    let mut script = vec![
        HOOK_MARKER_START.to_string(),
        "# TokenJar Transparent CLI Interceptor (Bash/Zsh)".to_string(),
    ];

    for cmd in DEFAULT_WRAPPED_COMMANDS {
        script.push(format!(
            "{cmd}() {{\n    if [ \"$RAW\" = \"1\" ] || [ \"$TOKENJAR_BYPASS\" = \"1\" ]; then\n        command {cmd} \"$@\"\n    else\n        tokenjar run -- {cmd} \"$@\"\n    fi\n}}"
        ));
    }
...
mod tests {
    ...
    fn test_generate_powershell_hook() {
        ...
        assert!(hook.contains("tokenjar run -- pytest"));
    }
