...
impl Default for TokenJarConfig {
    fn default() -> Self {
        Self {
...
impl TokenJarConfig {
    ...
    pub fn load_from_dir<P: AsRef<Path>>(root: P) -> Self {
        ...
        let candidates = [root.join("tokenjar.toml"), root.join(".tokenjar.toml")];

        for cand in &candidates {
            if cand.is_file() {
                if let Ok(text) = std::fs::read_to_string(cand) {
                    if let Ok(cfg) = toml::from_str::<TokenJarConfig>(&text) {
                        return cfg;
                    }
                }
            }
        ...
        let json_cand = root.join(".tokenjarrc");
        if json_cand.is_file() {
            if let Ok(text) = std::fs::read_to_string(json_cand) {
                if let Ok(cfg) = serde_json::from_str::<TokenJarConfig>(&text) {
                    return cfg;
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_is_ignored() {
        let cfg = TokenJarConfig::default();
