...
from token_saver.server import (
...
def test_mcp_resources():
    ...
    assert "Token-Saver" in stats_out or "No telemetry" in stats_out or "tokens" in stats_out.lower()

    guide_out = resource_guide()
    assert "Token-Saver AI Optimization Guidelines" in guide_out
...
def test_mcp_prompt():
    ...
    assert "Token-Saver AI Optimization Guidelines" in prompt_text
