...
def get_language(lang_name: str) -> tree_sitter.Language | None:
    ...
    try:
        ...
        print(f"[tokenjar] Language not available: {lang_name}", file=sys.stderr)
        return None
...
def get_parser(lang_name: str) -> tree_sitter.Parser | None:
    ...
    try:
        ...
        print(f"[tokenjar] Parser not available: {lang_name}", file=sys.stderr)
        return None
...
def parse_code(source: str, lang_name: str) -> tree_sitter.Tree | None:
    ...
    try:
        ...
        print(f"[tokenjar] Parse error for {lang_name}: {e}", file=sys.stderr)
        return None
